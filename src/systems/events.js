...
import {fail,success,random} from './common.js';
...
export function eventChoice(state,{choiceId}) {
  ...
  if (!EVENTS[id].choices.some(choice=>choice.id===choiceId)) return fail('invalid');
  if (choiceId==='brace'&&state.supplies.repair<3) return fail('notEnoughSupplies');
  if (choiceId==='pay'&&state.gold<450) return fail('funds',{cost:450});
  ...
  if (id==='pirates') {
    if (choiceId==='pay') {
      state.gold-=450;
      return success(state,'payPirates');
    }
    if (choiceId==='evade'&&random(state)<Math.min(.88,.4+stats.skills.navigation/300)) return success(state,'evaded');
