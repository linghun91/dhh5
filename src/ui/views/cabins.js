...
import { WEIGHTS, RANGES, CANNON_KINDS, cannonAt, cannonCard, batterySummary, rateLabel, emptyCannons } from '../naval.js';

function renderArmory(state,ship,stats,slot,ui={}) {
  ...
  const cannons=Object.values(CANNONS).filter(cannon=>(filter==='all'||cannon.range===filter)&&(kind==='all'||cannon.kind===kind)).sort((a,b)=>a.weight-b.weight||a.firepower-b.firepower);
  return `<section class="armory-section">${sectionTitle(`舱位 ${slot+1} · 火炮配装`,`<span class="badge gold">炮架承重：${WEIGHTS[type.maxGunWeight]}</span>`)}<div class="armory-current"><div>${art(current.asset,current.name)}<div><small>当前炮组</small><h3>${escape(current.name)}</h3><p>单独更换此炮台，其他炮台可保留不同炮型。</p></div></div><span class="badge">已装配</span></div><p class="hint">下方预览换装后的整船基础火力与齐射耗弹。换炮收取全价，旧炮不折现；新建炮台收取炮台费与默认炮价。承重、射程和弹耗均为游戏规则。</p><div class="archive-filters" aria-label="火炮距离筛选">${Object.entries({all:'全部距离',...RANGES}).map(([id,name])=>button(name,'cannon-filter',{filter:id},filter===id?'outline active':'ghost')).join('')}</div><div class="archive-filters" aria-label="火炮门类筛选">${Object.entries(CANNON_KINDS).map(([id,name])=>button(name,'cannon-kind',{kind:id},kind===id?'outline active':'ghost')).join('')}</div><div class="cannon-grid refit-grid">${cannons.map(cannon=>{
    ...
    return cannonCard(cannon,`${previewText}${button(`${reason}${selected?'':` · ${price(cannon.price)}`}`,'game',{type:'armCannon','ship-id':ship.id,slot,'cannon-id':cannon.id},selected?'ghost':compatible?'outline':'ghost',selected||!compatible||!state.portId||state.gold<cannon.price)}`,selected,cannon.id===ui.inspectedCannon);
  }).join('')||emptyCannons}</div></section>`;
}
