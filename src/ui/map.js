import { PORTS, SHIP_TYPES } from '../data/catalog.js';
...
export function renderMap(state, ui, route) {
  ...
  const flag=SHIP_TYPES[state.fleet[0]?.type]||SHIP_TYPES.caravel;
  ...
  }).join('')}</g><g class="fleet-marker" transform="translate(${location[0]},${location[1]})"><circle r="17"/><image href="./${flag.asset}" x="-27" y="-34" width="54" height="45"/></g></svg></div><div class="map-bottomline"><span><i></i>${UI.discovered}<i class="unknown"></i>${UI.unknown}</span><div class="map-controls">${button(icon('minus'),'map-zoom',{factor:1.25},'ghost tiny',false)}${button(icon('compass'),'map-reset',{},'ghost tiny')}${button(icon('plus'),'map-zoom',{factor:.8},'ghost tiny')}</div></div><p class="map-tip">${UI.chartHint}</p></section>`;
}
